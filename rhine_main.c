...
#else
static const struct ethtool_ops rhine_ethtool_ops;
#endif
#endif
...
rhine_get_options(POPTIONS pOpts, int index) {
    ...
    rhine_set_int_opt(&pOpts->tagging,enable_tagging[index],
...
void rhine_init_cam_filter(PRHINE_INFO pInfo) {
    if(pInfo->hw.sOpts.tagging != 2) {
        ...
        BYTE_REG_BITS_SET(&pInfo->hw, 0,(TCR_RTGOPT|TCR_PQEN),MAC_REG_TCR);
        BYTE_REG_BITS_OFF(&pInfo->hw, BCR1_VIDFR,MAC_REG_BCR1);        
    }
    //Disable all cam
...
rhine_init_adapter(PRHINE_INFO pInfo, RHINE_INIT_TYPE InitType) {
    ...
    printk(KERN_INFO "%s: rhinefet ver %s\n", pInfo->dev->name, RHINE_VERSION);
    ...
    default:
        ...
        if(byRevId >= REV_ID_VT6115_A0)  
        {   //For VT6115 pattern issue.
            pInfo->hw.sOpts.tx_thresh = 4;
            pInfo->hw.sOpts.rx_thresh = 7;
        }
...
rhine_found1(struct pci_dev *pcid, const struct pci_device_id *ent)
{
...
#ifdef RHINE_ETHTOOL_IOCTL_SUPPORT
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,4,25)
    dev->ethtool_ops = &rhine_ethtool_ops;
#endif
#endif
    ...
    if (pInfo->hw.byRevId >= REV_ID_VT6105M_A0) {
        if(pInfo->hw.byRevId < REV_ID_VT6115_A0) {
            pChip_info->flags |= (RHINE_FLAGS_TAGGING|RHINE_FLAGS_TX_CSUM|RHINE_FLAGS_HAVE_CAM|RHINE_FLAGS_RX_CSUM);
        }
        else {
            pChip_info->flags |= (RHINE_FLAGS_TAGGING|RHINE_FLAGS_HAVE_CAM|RHINE_FLAGS_RX_CSUM);
        }
    }
...
static int rhine_rx_srv(PRHINE_INFO pInfo, U32 status) {
    ...
    rhine_reset_rx_stats(&pInfo->hw.stats); //Reset rx temp statistics
    ...
    while (TRUE) {
        ...
        if (pInfo->hw.flags & RHINE_FLAGS_FLOW_CTRL)
        {
            ...
            else // For patching VT6115 RBRDU bug.
            {
                byRBRDU = CSR_READ_1(&pInfo->hw, MAC_REG_FLOWCR0);
                 if(byRBRDU < pInfo->hw.sOpts.nRxDescs)
                {
                    CSR_WRITE_1(&pInfo->hw, 1, MAC_REG_FLOWCR0);      
                }
            }
...
rhine_receive_frame(PRHINE_INFO pInfo, int idx) {
    ...
    //Get Tag
    if(pInfo->chip_id == 4) { //Check if is 6105M or not
        if (pRD->rdesc1 & cpu_to_le32(PQSTS_TAG)) {
            if (pInfo->hw.sOpts.tagging == 0) {
    ...
    /*if (pInfo->hw.flags & RHINE_FLAGS_VAL_PKT_LEN) {
        if(skb->protocol == htons(ETH_P_802_2)){
            ...
            if (skb->len != htons(*(PU16)(skb_mac_header(skb) + 12))){                
#else
            if (skb->len != htons(*(PU16)(skb->mac.raw + 12))){                
#endif        
                 //skb_put(skb, -(pRD->rdesc0.f15Length-4));
                skb_put(skb, -(frame_length-4));
                pStats->rx_length_errors++;
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,22) 
   skb_reserve(pRDInfo->skb, 16 - (((unsigned long)skb_tail_pointer(pRDInfo->skb)) & 15));
...
static void rhine_tx_srv_drop_all(PRHINE_INFO pInfo) {
    ...
    rhine_reset_tx_stats(&pInfo->hw.stats); //Reset tx temp statistics
...
static int  rhine_open(struct net_device *dev) {
    ...
    i=request_irq(pInfo->pcid->irq, (irq_handler_t)rhine_intr, IRQF_SHARED, dev->name, dev);
#else
...
#else
static void rhine_ethtool_gdrvinfo(struct net_device*  dev, struct ethtool_drvinfo  *info)
{
    PRHINE_INFO pInfo = netdev_priv(dev);
...
static int rhine_ethtool_gset(struct net_device*  dev, struct ethtool_cmd *ecmd)
{
    PRHINE_INFO pInfo = netdev_priv(dev);
    U32     status;
    unsigned long    flags;
    ...
    ecmd->speed = (status & RHINE_SPEED_100) ? SPEED_100 : SPEED_100;
    ecmd->autoneg = (status & RHINE_AUTONEG_ENABLE) ? AUTONEG_ENABLE : AUTONEG_DISABLE;
    ecmd->port = PORT_TP;
    ecmd->transceiver = XCVR_INTERNAL;
    ecmd->phy_address = CSR_READ_1(&pInfo->hw, MAC_REG_MIIAD) & 0x1F;
    ecmd->duplex = (status & RHINE_DUPLEX_FULL) ? DUPLEX_FULL : DUPLEX_HALF;
    
    return 0;
}

static int rhine_ethtool_sset(struct net_device*  dev, struct ethtool_cmd *ecmd)
{
    PRHINE_INFO pInfo = netdev_priv(dev);
    SPD_DPX_OPT connection_type=SPD_DPX_AUTO; 
    unsigned long    flags;
    ...
    {
        if(ecmd->speed == SPEED_100)
            connection_type = (ecmd->duplex == DUPLEX_FULL)?SPD_DPX_100_FULL:SPD_DPX_100_HALF;
        else
            connection_type = (ecmd->duplex == DUPLEX_FULL)?SPD_DPX_10_FULL:SPD_DPX_10_HALF;       
    }
...
static u32 rhine_ethtool_glink(struct net_device*  dev)
{
    PRHINE_INFO pInfo = netdev_priv(dev);
    int    link_ok = BYTE_REG_BITS_IS_ON(&pInfo->hw, MIISR_LNKFL,MAC_REG_MIISR) ? 0 : 1;
    return link_ok;
}
static u32 rhine_ethtool_gmsglvl(struct net_device*  dev)
{
    return msglevel;
}
static void rhine_ethtool_smsglvl(struct net_device*  dev, u32 value)
{
    msglevel = value;
}

static void rhine_ethtool_gwol(struct net_device*  dev, struct ethtool_wolinfo *wol)
{
    PRHINE_INFO pInfo = netdev_priv(dev);
...
static int rhine_ethtool_swol(struct net_device*  dev, struct ethtool_wolinfo *wol)
{
    PRHINE_INFO pInfo = netdev_priv(dev);
...
static const struct ethtool_ops rhine_ethtool_ops = {
    .get_drvinfo    =   rhine_ethtool_gdrvinfo,
    .get_settings    =   rhine_ethtool_gset,
    .set_settings    =   rhine_ethtool_sset,
    .get_link       =   rhine_ethtool_glink,
    .get_msglevel   =   rhine_ethtool_gmsglvl,
    .set_msglevel   =   rhine_ethtool_smsglvl,
    .get_wol        =   rhine_ethtool_gwol,
    .set_wol        =   rhine_ethtool_swol,
};
#endif
#endif
